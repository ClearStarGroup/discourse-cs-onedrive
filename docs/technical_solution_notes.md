...
- UI: Ember component plugged into `post-article-content` connector outlet, only rendered for the first post (`post_number === 1`); distinct states for "unauthenticated" (MSAL sign-in prompt), "no folder linked" (picker CTA), "folder linked with access" (file list + refresh/change/delete actions), and "folder linked but viewer lacks access" (error messaging). Styling should rely on core Discourse classes/variables to match the default themes out of the box.
...
- Auth: Import `@azure/msal-browser` directly from vendor assets in `onedrive-auth-service.js`, so the module is bundled with the plugin (no CDN dependency) and MSAL APIs are available synchronously. The service exports `msalLibrary`, `InteractionRequiredAuthError`, and `getRedirectUri()` for use by other services. Note: MSAL is stored in two locations—`assets/javascripts/vendor/` (for Ember bundling) and `public/javascripts/vendor/` (for direct script loading in the OAuth callback page).
...
2. Copy the browser bundles from `node_modules` into the plugin's vendor directories:
   - **MSAL (two copies required):**
     - `cp node_modules/@azure/msal-browser/lib/msal-browser.min.js assets/javascripts/vendor/msal-browser.min.js` (for Ember bundling)
     - `cp node_modules/@azure/msal-browser/lib/msal-browser.min.js public/javascripts/vendor/msal-browser.min.js` (for OAuth callback page)
   - **Graph Client:**
...
**Note on MSAL duplication:** The plugin maintains two copies of `msal-browser.min.js`:

- `assets/javascripts/vendor/msal-browser.min.js` is imported directly by `onedrive-auth-service.js` and bundled into the main JavaScript bundle.
- `public/javascripts/vendor/msal-browser.min.js` is served as a static file and loaded directly via `<script>` tag in the OAuth callback page (`callback.html.erb`), which needs MSAL available synchronously before the inline script runs. Both copies must be kept in sync when updating the library version.
